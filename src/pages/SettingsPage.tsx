...
import { Check, XCircle, Play, Settings as SettingsIcon, Globe, Users, Bolt, Sparkles, Download, Lock } from "../components/Icons";
...
const SETTINGS_API_URL = import.meta.env.VITE_API_URL || "https://api.flow-vid.com";
...
type SettingsTab = "general" | "playback" | "subtitles" | "social" | "addons" | "subscription" | "about" | "account";

const TABS: { id: SettingsTab; label: string; Icon: React.FC<{ size?: number }> }[] = [
  { id: "general", label: "General", Icon: SettingsIcon },
  { id: "playback", label: "Playback", Icon: Play },
  { id: "subtitles", label: "Subtitles", Icon: Globe },
  { id: "social", label: "Social", Icon: Users },
  { id: "addons", label: "Addons & Data", Icon: Bolt },
  { id: "subscription", label: "Subscription", Icon: Sparkles },
  { id: "about", label: "Update", Icon: Download },
      ...
      <div className="settings-content">
        ...
        {activeTab === "general" && (
          <section className="settings-section">
            <p className="section-description">Customize how content is displayed.</p>

            <div className="setting-item">
              <div className="setting-info">
                <label>Blur Unwatched Episode Thumbnails</label>
                <p>Blur episode images to avoid spoilers until you start watching</p>
              </div>
              <button
            ...
            <div className="setting-item">
              <div className="setting-info">
                ...
                <p>Display personalized recommendations on the home page based on your library</p>
              </div>
              <button
          ...
          <section className="settings-section">
            ...
            <div className="setting-item">
              <div className="setting-info">
                ...
                <p>Choose your preferred video player. MPV offers better codec support.</p>
              </div>
              <select
                className="select"
                value={playerType}
              ...
              >
                ...
                <option value="embedded-mpv">MPV Player (Better codec support)</option>
              </select>
            </div>
            ...
            <div className="setting-item">
              <div className="setting-info">
                ...
                <p>"Best quality first" sorts all streams by quality across addons. "Group by addon" keeps streams grouped by their addon order.</p>
              </div>
              <select
                className="select"
                value={streamSorting}
        ...
        {activeTab === "subtitles" && (
          <section className="settings-section">
            <p className="section-description">Configure automatic subtitle loading and language preferences.</p>
            ...
            <div className="setting-item">
              ...
              <button
                ...
                onClick={() => setPreferHearingImpaired(!subtitles.preferHearingImpaired)}
              >
                <span className="toggle-handle" />
              </button>
            </div>
                ...
                <div className="subtitle-preview-video">
                  <div
                    ...
                    style={{
                      ...
                      backgroundColor: (() => {
                        const hex = (subtitleAppearance.backgroundColor ?? "#000000").replace("#", "");
              ...
              <div className="appearance-controls">
                <div className="appearance-row">
                  ...
                  <div className="slider-with-value">
                    <input type="range" min="16" max="36" value={subtitleAppearance.fontSize ?? 28} onChange={(e) => setSubtitleAppearance({ fontSize: parseInt(e.target.value) })} />
                ...
                <div className="appearance-row">
                  ...
                  <select className="input select-input" value={subtitleAppearance.fontFamily} onChange={(e) => setSubtitleAppearance({ fontFamily: e.target.value })}>
                ...
                <div className="appearance-row">
                  ...
                  <input type="color" value={subtitleAppearance.textColor} onChange={(e) => setSubtitleAppearance({ textColor: e.target.value })} className="color-input" />
                </div>

                <div className="appearance-row">
                  <label>Background Color</label>
                  <input type="color" value={subtitleAppearance.backgroundColor} onChange={(e) => setSubtitleAppearance({ backgroundColor: e.target.value })} className="color-input" />
                </div>

                <div className="appearance-row">
                  <label>Background Opacity</label>
                  <div className="slider-with-value">
                    <input type="range" min="0" max="100" value={Math.round((subtitleAppearance.backgroundOpacity ?? 0.6) * 100)} onChange={(e) => setSubtitleAppearance({ backgroundOpacity: parseInt(e.target.value) / 100 })} />
                    <span>{Math.round((subtitleAppearance.backgroundOpacity ?? 0.6) * 100)}%</span>
                  </div>
                </div>

                <div className="appearance-row">
                  <label>Text Shadow</label>
                  <button className={`toggle ${subtitleAppearance.textShadow ? "active" : ""}`} onClick={() => setSubtitleAppearance({ textShadow: !subtitleAppearance.textShadow })}>
                    <span className="toggle-handle" />
                  </button>
                </div>
                ...
                <div className="appearance-row">
                  ...
                  <div className="slider-with-value">
                    <input type="range" min="12" max="20" step="1" value={Math.round((subtitleAppearance.lineHeight ?? 1.4) * 10)} onChange={(e) => setSubtitleAppearance({ lineHeight: parseInt(e.target.value) / 10 })} />
                    <span>{(subtitleAppearance.lineHeight ?? 1.4).toFixed(1)}x</span>
                  </div>
                </div>
                ...
                <div className="appearance-row">
                  ...
                  <div className="slider-with-value">
                    <input type="range" min="5" max="25" value={subtitleAppearance.bottomPosition ?? 10} onChange={(e) => setSubtitleAppearance({ bottomPosition: parseInt(e.target.value) })} />
        ...
        {activeTab === "social" && (
          <section className="settings-section">
            <p className="section-description">Control your social features and privacy.</p>
            <FriendCodeDisplay />
            <div className="setting-item">
              <div className="setting-info">
                <label>Friends</label>
                <p>View your friends, send & receive friend requests, and manage your friends list</p>
              </div>
              <button className="btn btn-secondary" onClick={() => navigate("/friends")}>
                Open Friends
              </button>
            </div>
            <ActivityVisibilityToggle />
          </section>
        )}
        ...
        {activeTab === "addons" && (
          <section className="settings-section">
            <p className="section-description">Manage addons and metadata providers.</p>
            ...
            <div className="setting-item">
              ...
              <button className="btn btn-secondary" onClick={() => navigate("/addons")}>
                Manage Addons
              </button>
            </div>
            ...
            <h3 className="settings-sub-heading">TMDB Metadata</h3>
            <p className="section-description">
              TMDB provides rich metadata like cast photos, trailers, production companies, and recommendations.
              A built-in API key is included — provide your own for better rate limits.
            </p>
            ...
            <div className="setting-item">
              <div className="setting-info">
                ...
                <p>
                  Get a free key at{" "}
                  <a href="https://www.themoviedb.org/settings/api" target="_blank" rel="noopener noreferrer" style={{ color: "var(--primary)" }}>
                    themoviedb.org
                  </a>
                </p>
              </div>
              <div style={{ display: "flex", gap: "8px", alignItems: "center" }}>
                <input
                  ...
                  value={tmdbKeyInput}
                  onChange={(e) => { setTmdbKeyInput(e.target.value); setTmdbValidation(null); }}
                  style={{ width: "260px" }}
                />
                <button className="btn btn-secondary btn-sm" onClick={handleTmdbSaveKey} disabled={tmdbValidating || !tmdbKeyInput.trim()}>
                  {tmdbValidating ? "Validating..." : "Save"}
                </button>
                {tmdbUseCustomKey && tmdbCustomApiKey && (
                  <button className="btn btn-ghost btn-sm danger" onClick={handleTmdbRemoveKey}>Remove</button>
                )}
              </div>
            </div>
            ...
            <div className="setting-item">
              ...
              <button className="btn btn-secondary btn-sm" onClick={handleTmdbClearCache}>
    ...
    return (
      <div className="settings-row">
        <div className="settings-row-info">
          ...
          <p>Log in to manage account deletion</p>
        </div>
      </div>
    );
  }
...
function FriendCodeDisplay() {
  ...
  useEffect(() => {
    authenticatedFetch(`${SETTINGS_API_URL}/friends/code`)
      .then((res) => res.json())
      .then((data) => setFriendCode(data.data?.friendCode || null))
      .catch(() => {});
  }, []);
  ...
  return (
    <div className="setting-item">
      <div className="setting-info">
        ...
        <p>Share this code with others so they can add you as a friend</p>
      </div>
      <button className="friend-code-btn" onClick={handleCopy} title="Click to copy">
        <span className="friend-code-text">{friendCode.length >= 8 ? `${friendCode.slice(0, 4)}-${friendCode.slice(4)}` : friendCode}</span>
        <span className="friend-code-hint">{copied ? "Copied!" : "Copy"}</span>
      </button>
    </div>
  );
}
...
function ActivityVisibilityToggle() {
  const [visible, setVisible] = useState(true);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    authenticatedFetch(`${SETTINGS_API_URL}/friends/activity-visibility`)
      .then((res) => res.json())
      .then((data) => setVisible(data.data?.activityVisible ?? true))
      .catch(() => {})
  ...
  const handleToggle = async () => {
    ...
    setVisible(newVal);
    await authenticatedFetch(`${SETTINGS_API_URL}/friends/activity-visibility`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ visible: newVal }),
    }).catch(() => setVisible(!newVal));
  };
  ...
  return (
    <div className="setting-item">
      <div className="setting-info">
        <label>Show Activity to Friends</label>
        <p>Let your friends see what you're currently watching</p>
      </div>
      <button
